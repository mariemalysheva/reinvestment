...
contract AssetNFT is ERC1155 {
    ...
    event userBatchTransferred(address to_, uint256[] ids_, uint256[] amounts_);
    event userBatchAssetsMinted(uint256[] ids_, uint256[] amounts_);

    constructor(address manager_, address vault_, uint256 reinvestmentPeriod_) ERC1155 (""){
    ...
    function mintNFTBatch(address assetVault, uint256[] memory userIds_,  AssetMetadata[] memory metadata_) public onlyAdmin {
    ...
    function transferUserAssets(uint256[] memory userIDs_, address to_) public onlyAdmin{
        uint256 n = userIDs_.length;
        ...
        for (uint256 j = 0; j<_max_j; j++){
            ...
            emit userBatchTransferred(to_, ids_, amounts_);
        }
    }

...
contract ReinvestmentManager  {
    ...
    event userBatchAdded(uint256 reinvestmentPeriodID, AssetNFT.AssetMetadata[] metadata);
    event savingsReinvested(ReinvestmentPeriod reinvestmentPeriod, uint256[] userIDs, uint256[] newBalances);
    event userBatchTransferred(uint256[] userIDs, address to);


    function addUserBatch(uint256[] memory userIDs_, string[] memory salt_, uint256[] memory balance_) external  onlyAdmin{
        require(userIDs_.length == balance_.length, "user and balance arrays not of the same length");
        require(userIDs_.length == salt_.length, "user and salt arrays not of the same length");

        uint len = userIDs_.length;
        AssetNFT.AssetMetadata[] memory metadata_ = new AssetNFT.AssetMetadata[](len);
    ...
    function newReinvestmentPeriod() internal {
        _reinvestmentID += 1;
        _currentAsset = new AssetNFT(address(this), msg.sender, _reinvestmentID);
    ...
    function transferUserBatch(uint256[] memory userIDs_, address to_) external {
        ...
        emit userBatchTransferred(userIDs_, to_);
    }
